...
pub(super) fn decode_query<T>(query: Result<Query<T>, QueryRejection>) -> Result<T, Error> {
    query
...
mod tests {
    ...
    async fn actual_control_routes_reject_unknown_queries_without_a_database() {
        ...
        let app = super::super::build(config, store);
        ...
        for path in [
            ...
            let response = super::super::tests::request(
                app.clone(),
                format!(
